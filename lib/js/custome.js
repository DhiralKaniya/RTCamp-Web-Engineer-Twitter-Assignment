	...
	$.ajax({
		...
		success:function(result){
			...
			$.each(result['data'],function(index,data){
				$('<div class="item text-center slider_item" value='+data['text']+'>'+data['text']+'</div>').appendTo('.carousel-inner');
			});
...
$(document).on('click','.follower, .suggestion_follower',function(){
	...
	$.ajax({
			...
			success:function(result){
				...
				$.each(result['data'],function(index,data){
					if(data['images']!=null){
						$('<div class="item text-center slider_item">'+data['text']+'<br><img src='+data['images']+'></div>').appendTo('.carousel-inner');
					}else{
